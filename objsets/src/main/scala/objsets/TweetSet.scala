...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if(l_count > r_count && l_count > elem.retweets) left.mostRetweeted
    else if(r_count > elem.retweets) right.mostRetweeted
    else elem
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = 
    TweetReader.allTweets.filter(t => t.text.exists(s => google.contains(s)))


  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter(t => t.text.exists(s => apple.contains(s)))
